...
public class ChargeUtil {
    ...
    public ChargeUtil(String appId) {
        this.appId = appId;
    }

    public static void runDemos(String appId) {

        ChargeUtil chargeExample = new ChargeUtil(appId);
        System.out.println("------- 创建 charge -------");
        Charge charge = chargeExample.createCharge(null);
     ...
     * @return Charge
     */
    public Charge createCharge(PxxVo pxxVo) {
        Charge charge = null;
        String channel = pxxVo.getChannel();
        Map<String, Object> chargeMap = new HashMap<String, Object>();
        chargeMap.put("amount", pxxVo.getAmount());//订单总金额, 人民币单位：分（如订单总金额为 1 元，此处请填 100）
        chargeMap.put("currency", "cny");
        chargeMap.put("subject", "Your Subject");
        chargeMap.put("body", "Your Body");
        chargeMap.put("order_no", pxxVo.getOrder_no());// 推荐使用 8-20 位，要求数字或字母，不允许其他字符
